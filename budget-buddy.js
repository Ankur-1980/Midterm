...
btn.addEventListener("click", () => {
  const amountInputValue = document.getElementById("amount-input").value;
  const money = document.getElementById("balance-text").innerHTML;
  const moneySplit = money.split("$");
  let amount = parseFloat(moneySplit[1]);
...
function addTransactionDOM(transaction) {
  ...
  item.innerHTML = `${transaction.desc} <span>${sign}${Math.abs(
    transaction.amount
  )}</span> <span>${
